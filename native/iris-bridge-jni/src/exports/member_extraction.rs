...
#[unsafe(no_mangle)]
pub extern "system" fn Java_party_qwer_iris_imagebridge_runtime_core_BridgeCoreJniMemberField_nativeMemberProfileUserIds<
    'local,
>(
    mut env: JNIEnv<'local>,
    _class: JClass<'local>,
    ...
    catch_jstring(&mut env, |env| {
        let request_json = read_string(env, &request_json);
        return_string(env, &dispatch_member_profile_user_ids(&request_json))
    })
}

#[unsafe(no_mangle)]
pub extern "system" fn Java_party_qwer_iris_imagebridge_runtime_core_BridgeCoreJniMemberField_nativeMemberProfilePayload<
    'local,
>(
    mut env: JNIEnv<'local>,
    _class: JClass<'local>,
    ...
    catch_jstring(&mut env, |env| {
        let request_json = read_string(env, &request_json);
        return_string(env, &dispatch_member_profile_payload(&request_json))
    })
}
